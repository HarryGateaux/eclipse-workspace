...
//		System.out.print(lastClicked);
		if (lastSelected != null && lastClicked == null) {
			hideMarkers(lastSelected);
	...
	private void threatenedCities(EarthquakeMarker m) {

		for (Marker city : cityMarkers) {
			if (m.getDistanceTo(city.getLocation()) < m.threatCircle()) {
				city.setHidden(false);
			}
		}
	}
	
	private void threateningQuakes(Marker m) {

		for (Marker q : quakeMarkers) {
			EarthquakeMarker quake = (EarthquakeMarker) q;
			if (m.getDistanceTo(quake.getLocation()) < quake.threatCircle()) {
				quake.setHidden(false);
			}
		}
	}

	// hide all markers except the one passed
	private void hideMarkers(Marker m) {
		...
		for (Marker marker : cityMarkers) {
			if (marker != m) {
				...
				// code that displays earthquakes

			}

		}

		// check for cities/quakes in threat zone and unhide them
		if (m instanceof EarthquakeMarker) {
			threatenedCities((EarthquakeMarker) m);
		}
		
		if (m instanceof CityMarker) {
			threateningQuakes((CityMarker) m);
		}
