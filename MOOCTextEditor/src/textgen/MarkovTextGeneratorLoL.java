...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText) {
		...
		{	for (int i = 0; i < words.length; i++) {
				...
				if (nodeCheck(words[i]) == null) {
					...
					if (i != words.length - 1) {
						nodeCheck(words[i]).addNextWord(words[i + 1]);

					} else

					{
						nodeCheck(words[i]).addNextWord(starter);
					}
	...
	public String generateText(int numWords) {
		...
		for (int i = 1; i < numWords; i++) {
			...
			String randomNextWord = nodeCheck(starter).getRandomNextWord(random);
			output += " " + randomNextWord;
			starter = nodeCheck(randomNextWord).getWord();
		}
	...
	private ListNode nodeCheck(String word) {
